...
| 기차 | `train` | 🟣 #8b5cf6 | 직선 (고정 철도 노선) / **TODO: 코레일 API 연동 예정** |
...
### 진행 예정
- [ ] **기차 API 연동** ← 다음 작업
  - 공공데이터포털 코레일 기차 시간표 API 조사 필요
  - 출발역/도착역 자동 매칭 (좌표 → 가장 가까운 역 탐색)
  - `longDistance: true` 구간에서 기차 경로 자동 표시 (일직선 → 실제 역 경유 폴리라인)
  - 소요시간·요금 정보 표시
